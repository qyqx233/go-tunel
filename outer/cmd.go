...
func (c *cmdServer) cmdLoop(wc lib.WrapConnStru, shake *proto.ShakeProto, t *transportStru) {
	logger.Info("开始回应命令通道")
	...
	err := shake.Send(wc)
	if err != nil {
		logger.Errorf("响应命令通道握手信息失败 %v", err)
		return
	}
	...
	if !t.proxyStarted {
		...
		if err != nil {
			logger.Errorf("监听端口失败 %v", err) // 如果监听失败，则直接关闭链接，等待下一次重试
			return
		}
		t.proxyStarted = true
	}
	...
	for {
		...
		if cmd.Usage == proto.BeatUsage {
			logger.Infof("接收到通道%d发过来的心跳", wc.ID())
			cmd.Code = proto.OkCode
			err = cmd.Send(wc)
			if err != nil {
				logger.Infof("回应通道%d心跳失败", wc.ID())
				return
			}
		}
	}
...
func (c *cmdServer) dispatch(conn net.Conn, shake *proto.ShakeProto, t *transportStru) (bool, error) {
	var needClose = true
	if shake.Type == proto.TransportType { // 如果是传输通道
		...
		if shake.Usage == proto.InitiativeTransportUsage {
			...
			logger.Infof("获取到一个回应通道%s，reqId=%d, 原reqID=%d", conn.RemoteAddr().String(),
